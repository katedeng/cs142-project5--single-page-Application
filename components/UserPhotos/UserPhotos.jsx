...
class UserPhotos extends React.Component {
    ...
    render() {
        return this.user ? ( <
            ...
            Grid item xs = { 12 } >
            <
            Typography variant = "h3" > { ' ' } { this.user.first_name } { this.user.last_name } & apos; s photos { ' ' } <
            /Typography>{' '} <
            /Grid>{' '} {
                this.state.photos ? (
                            ...
                            photo.comments ? (
                                photo.comments.map((comment) => {
                                    return ( <
                                        ...
                                        <
                                        Grid item xs = { 2 } > { ' ' } { comment.date_time } { ' ' } <
                                        /Grid>{' '} <
                                        Grid item xs = { 2 } >
                                        <
                                        Link to = { `/users/${comment.user._id}` } > { ' ' } { `${comment.user.first_name} ${comment.user
																	.last_name}` } { ' ' } <
                                        /Link>{' '} <
                                        /Grid>{' '} <
                                        Grid item xs = { 8 } > { ' ' } { comment.comment } { ' ' } <
                                        /Grid>{' '} <
                                        /Grid>
                                    );
                                })
